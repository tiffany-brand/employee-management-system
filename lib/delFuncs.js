...
const delRole = async () => {
    try {
        ...
        if (roleEmps.length) {
            console.log("\n------------------------\n")
            console.log("This role is assigned to active employees.\nYou must assign these employees different roles before you can remove this role:\n")
            roleEmps.forEach((emp) => {
                console.log(`${emp.first_name} ${emp.last_name}`)
            })
            console.log("\nChoose the 'Update Employee Role' task.")
            console.log("\n------------------------\n")
        } else {
            ...
            const confirm = await promptUser([
                {
                    ...
                    type: "confirm",
                    default: false,
                    message: `\nAre you sure you want to remove ${removeRole[0].title}?`
                }
            ]);
            if (confirm.yN) {
